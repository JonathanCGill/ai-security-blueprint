...
!!! abstract "The foundation for MASO (Multi-Agent Security Operations)"
    This declare-constrain-evaluate pattern is the foundation of the [MASO Framework](maso/README.md). MASO extends it across 11 control domains, covering identity, data protection, extraction integrity, execution control, observability, supply chain security, privileged-agent governance, model cognition assurance, agentic task mandates, objective intent, and prompt/goal/epistemic integrity. If you are deploying agents at scale, start here to understand the principles, then move to MASO for the full control set.
