...
| **Over-evaluation** | A cloud judge on 100% of actions including read-only ones; naive MASO (Multi-Agent Security Operations) where security overhead runs 3 to 5x generator cost | Risk-gated evaluation routing and SLM sidecars (see [Token Economics](token-economics.md#risk-as-the-evaluation-gate)) |
