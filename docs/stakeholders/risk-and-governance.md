...
| **Multi-agent security** | Standards address single AI systems. They don't cover agent-to-agent communication, delegated authority, tool access chains, or emergent behavior in multi-agent workflows. | MASO (Multi-Agent Security Operations): eleven control domains for multi-agent orchestration with risk-tiered implementation |
