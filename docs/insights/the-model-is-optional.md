...
One control sits in neither column and deserves separate mention. [PG-2.2 goal integrity monitoring](../maso/controls/prompt-goal-and-epistemic-integrity.md) compares what the agent is *doing* against the declared objective, continuously. It catches all five disclosures, not because it understands any of them, but because none of them produce behaviour that matches the task the agent was given. Comparing actions to declared intent is the one semantic control that does not need a model turn to fire.
