...
**See also:** [MASO (Multi-Agent Security Operations) Objective Intent](../maso/controls/objective-intent.md), which extends the intent concept from workflow-level post-execution evaluation to per-agent, per-judge, and per-workflow behavioral mandates with tactical, strategic, and judge-monitoring evaluation levels.
