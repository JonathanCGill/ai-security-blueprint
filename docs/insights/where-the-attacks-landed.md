...
[IA-2.3](../maso/controls/identity-and-access.md), no transitive permissions, is the most frequently implicated control in six months of incidents, and it is never the control that was attacked. It is the control that was absent when something else was.
...
| [IA-2.3](../maso/controls/identity-and-access.md), no transitive permissions | **Held** | Predicted the blast-radius pattern in the Bedrock gateway, Hugging Face, Mastra, and Langflow incidents, and present long before any of them |
...
**The orchestration plane becomes the primary way in.** Two Langflow entries a year apart, both as initial access. Low-code agent builders concentrate model keys, database credentials, and connector tokens on hosts stood up by teams who do not think of themselves as running production infrastructure, and exploitation of CVE-2026-9198 began before the fix existed. Carried by ET-30 extended to the orchestration plane, [IA-2.4](../maso/controls/identity-and-access.md) vaulted per-flow credential brokering, and the orchestrator entering the asset inventory with a patch SLA.

**Regulation lands on observability, where the market ships nothing.** EU AI Act Article 12 became enforceable on 2 August with a six-month retention floor and a reconstructability standard, and Recitals 99 and 100 extend the boundary to every agent in a chain. RSAC 2026 established that no major vendor ships an agent behavioural baseline. That gap is now a compliance gap as well as a security one. Carried by the decision chain log and decision trace in [Observability](../maso/controls/observability.md), which already record tool invocations, delegations, memory writes, and approvals.
...
Of everything in the record, one item is simultaneously a demonstrated attack surface, an enforceable legal obligation, and a capability nobody sells: knowing what your agents normally do.

Embedding-based detection collapses under self-mixing. Output-only inspection misses the majority of tool-argument leakage. Retrospective log review took 141,006 runs to find three breaches. No vendor at RSAC shipped a behavioural baseline. And since 2 August, a high-risk deployment in the EU must be able to reconstruct what its agents did, with a six-month retention floor, across every agent in the chain.
