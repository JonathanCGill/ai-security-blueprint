...
Embedding-based detection collapses under self-mixing. Output-only inspection misses the majority of tool-argument leakage. Retrospective log review took 141,006 runs to find three breaches. No vendor at RSAC shipped a behavioural baseline. And the Article 12 reconstructability standard a high-risk EU deployment has to meet, across every agent in a chain, is now due on 2 December 2027 rather than this month, which is long enough to build it and short enough that starting late is the same as failing.
