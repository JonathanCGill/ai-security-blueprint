...
| [Privileged Agent Governance](../maso/controls/privileged-agent-governance.md) | MASO (Multi-Agent Security Operations) controls for the Judge as a privileged agent: calibration testing (PA-2.2), criteria versioning (PA-2.3), model rotation (PA-3.4), continuous calibration (PA-3.5). |
