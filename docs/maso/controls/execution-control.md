...
| **EC-2.5** Model-as-Judge gate | Evaluation model reviews all agent outputs before external commit | Evaluates context-dependent behaviour: policy compliance, goal integrity (cross-ref [PG-2.2](prompt-goal-and-epistemic-integrity.md)), output safety, data leakage. Judge criteria include faithfulness and analytical depth, not just format compliance (GV-02). Factual claims should be checked by independent verification against the source (knowledge-graph or API lookups), not asserted by the Judge alone, which is the least independent [reviewing control](../../core/controls.md). Match each criterion to the control that can verify it. |
...
| **EC-2.18** Egress path validation | Every network-access path available to a capable agent is enumerated and verified as intended before the agent runs, not assumed from configuration | The agent's actual reachable network is tested against its declared allow-list at start-up, and a mismatch fails closed. An unintended internet path is treated as a live incident, not a benign default. This is the pre-flight that would have removed the misconfiguration behind the July 2026 evaluation breaches ([INC-18](../threat-intelligence/incident-tracker.md#inc-18-anthropic-cybersecurity-evaluation-breaches-2026)), where machines believed to be air-gapped in fact had live internet access. Complements EC-2.2 (sandboxed execution) by verifying the sandbox's network boundary rather than trusting it. |
| **EC-2.20** Scenario and content egress validation | Every hostname, domain, and identifier appearing in the content given to a capable agent, including fictional targets in test scenarios, is resolved against real DNS and registry data before the run, and fails closed if it is routable | Egress validation is not only a network-configuration exercise. Meta's Muse Spark evaluation escape ([INC-20](../threat-intelligence/incident-tracker.md#inc-20-meta-muse-spark-evaluation-escape-2026), August 2026) happened because a fictional target name in the scenario coincided with a real registered domain, which connected an otherwise isolated environment to the public internet. Reserve non-routable names for scenario content (`.invalid`, `.test`, or an owned sink domain), and treat any resolvable name in scenario text as a live target. Complements EC-2.18, which validates the network boundary; this validates the content that tells the agent where to point. |
| **EC-2.19** Config-file immutability | Files that determine what the agent may execute (its MCP server manifest, hook definitions, task runners, tool allow-list) are write-protected from the agent itself; changes require host-enforced human approval | Execution-sensitive paths such as `mcp.json`, `.vscode/tasks.json`, hook configs, and `.cursorrules`/`CLAUDE.md`-style instruction files are enforced as policy at the host, not left to the agent's file-write tool. An agent that can silently edit the file listing its own tools can grant itself new ones; the AWS Kiro flaw (CVE-2026-10591) was exactly this, a hidden web prompt rewriting `mcp.json` to auto-launch an attacker-controlled MCP server. Pairs with [Supply Chain](supply-chain.md) config-file provenance, which treats the same files as untrusted when read; this control keeps the agent from writing them. |
...
**Assuming the egress boundary instead of validating it.** A configuration that says an environment has no internet access is not the same as an environment that has no internet access. In the July 2026 Anthropic evaluation breaches, the models were told they were air-gapped, the config implied it, and a misconfiguration left a live internet path open anyway; the agents followed that path into real production systems. Enumerate and test every reachable network destination against the declared allow-list at start-up (EC-2.18) and fail closed on any mismatch. Trusting the declared boundary is how an offensive-capable agent ends up on the open internet believing it is still in a simulation.

**Validating the network but not the scenario.** An environment can be correctly isolated at the network layer and still leak through the text you hand the agent. A fictional company or domain invented for a test scenario may be a real registered domain, and an agent given a reachable target will attack it, which is how Meta's Muse Spark reached and exploited a third-party service in August 2026. Resolve every name in the scenario before the run (EC-2.20) and use reserved non-routable namespaces for fictional targets.

**Letting the agent write its own execution policy.** The file that lists an agent's MCP servers, hooks, or task runners is execution policy, not ordinary configuration. If the agent's file-write tool can modify it without human approval, the agent can grant itself new tools, and any prompt injection that reaches the agent can too. The AWS Kiro flaw (CVE-2026-10591) was a hidden web prompt rewriting `mcp.json` to auto-launch an attacker's MCP server with the developer's privileges. Enforce write-protection on execution-sensitive paths at the host (EC-2.19); do not rely on the agent to decline.
