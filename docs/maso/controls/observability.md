...
| **OB-2.3** Drift detection | Statistical comparison of agent **action** distributions against a rolling baseline, with output distributions tracked separately as a quality signal | The security baseline is over the action vocabulary, not what the agent says: tool call mix, destinations resolved and which are new, sequence length and order, sub-agent fan-out and the number of servers contributing to one working context, egress volume and shape, and blocked tool attempts ([AT-1.6](agentic-task-mandate.md)). Content-based baselines degrade as the system becomes more agentic: embedding similarity between inter-agent messages collapses under self-mixing, and the majority of sensitive-data leakage travels in tool arguments that output inspection never sees. Output drift still matters for quality degradation, topic drift, and style change, but it is a separate measurement. Window: 7-day rolling baseline, alert on >2σ deviation. |
...
| **EU AI Act Art. 12** | Automatic recording of events over the system's lifetime, covering risk-relevant situations, post-market monitoring, and deployer operational monitoring. The Digital Omnibus (Regulation (EU) 2026/1744, published 24 July 2026) deferred the high-risk obligations to 2 December 2027 for stand-alone Annex III systems and 2 August 2028 for AI in Annex I products; Article 50 transparency applied from 2 August 2026 | The decision chain log and trace record the events that actually carry risk in an agentic system: tool invocations, delegations, memory writes, judge verdicts, and approval decisions. A prompt-and-completion log does not meet the reconstructability standard. Recitals 99 and 100 extend the boundary to every agent in a chain performing a high-risk function, so sub-agent traces are compliance artefacts, not debugging output |
