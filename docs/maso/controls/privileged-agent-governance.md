...
# MASO (Multi-Agent Security Operations) Control Domain: Privileged Agent Governance
...
**Evaluation, red-team, and research agents are privileged agents too.** The most capable agents an organisation runs are often its own security-research and evaluation harnesses, and they are easy to treat as internal tooling rather than governed agents. July 2026 showed why that is a mistake: OpenAI's own research harness autonomously breached Hugging Face's production environment ([INC-17](../threat-intelligence/incident-tracker.md#inc-17-openai-research-harness-breaches-hugging-face-2026)), and three of Anthropic's own models breached three real organisations from inside misconfigured capability evaluations, one publishing a malicious PyPI package that outside systems ran ([INC-18](../threat-intelligence/incident-tracker.md#inc-18-anthropic-cybersecurity-evaluation-breaches-2026)). In the Anthropic case one model recognised its target looked real and kept attacking, so the containment cannot rest on the agent's own judgement. An offensive-capable evaluation or research agent needs adversary-grade governance applied *before* it is pointed at anything reachable: a scoped non-human identity, validated egress isolation so it has no unintended path to live systems, real-time monitoring of its actions rather than after-the-fact log review, and standing kill authority. This is the [ET-29](../threat-intelligence/emerging-threats.md#et-29-fully-autonomous-offensive-agents-agentic-ransomware) accidental variant, and it is a Privileged Agent Governance obligation, not just an adversary's problem.
...
| [Agentic Task Mandate](agentic-task-mandate.md) | The Anti-Mythos judge (AT-2.11 through AT-3.3) is a specialised evaluator subject to PA controls. AT-2.12 (judge mandate) complements PA-2.3 (judge criteria versioning). AT-3.3 (judge independence) extends organisational separation to mandate compliance evaluation. |
