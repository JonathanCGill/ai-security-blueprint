...
# MASO (Multi-Agent Security Operations) Control Domain: Identity & Access
...
| **IA-2.4** Vaulted per-flow credential brokering | Orchestrators, gateways, and low-code agent builders hold no standing provider keys, database credentials, or connector tokens; each flow requests a short-lived, scoped token at execution time | An agent orchestrator accumulates every secret its flows need and becomes the highest-value host in the estate. The Langflow chain exploited in the wild ([CVE-2026-9198](../threat-intelligence/incident-tracker.md#inc-21-langflow-orchestrator-remote-code-execution-exploited-2026), August 2026) turned one unauthenticated RCE into access to every system its flows touched. Brokering credentials per flow means a compromised orchestrator yields a host rather than a keyring, and it gives the revocation and expiry that a stored key never has. |
