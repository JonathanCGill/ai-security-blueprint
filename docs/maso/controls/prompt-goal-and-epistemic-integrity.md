...
| **PG-2.11** Assembled-context evaluation | The Judge evaluates the agent's fully assembled working context immediately before an action, not only the individual messages that fed it | Per-message and per-source checks cannot see an instruction that exists only after fragments are combined. *GhostSplice* (August 2026) split a refused request across an MCP tool description, a tool result, and a sampling message, and across separate servers, so no fragment was flaggable alone. Evaluate the aggregate: what has this agent actually accumulated, from how many independent sources, and does it now amount to an instruction nobody sent? Cap the number of independently sourced MCP servers or tools that may contribute to one working context, and re-evaluate on every material context addition, not just at turn start. |
| **PG-2.12** Control-reasoning non-disclosure | A refusal states that an action is not permitted and stops; the reasoning behind a security control is never a user-facing output | Helpfulness training pushes models to justify refusals, and a justification about a control is a description of that control. Varonis found the undocumented `autorun=1` parameter behind CoSnitch (CVE-2026-24301, August 2026) by repeatedly asking Copilot *why* an attack would fail. Enforce a fixed refusal template at the output boundary rather than instructing the model to be vague, and treat a sequence of related refusals within one session as an OB-2.2 probing signal rather than a satisfied control. |
...
**Scanning each channel and calling the context clean.** A guardrail that clears every message, tool description, and tool result individually has established nothing about what those pieces mean together. *GhostSplice* (August 2026) is the proof: fragments that are innocuous apart, and that can arrive from different MCP servers, compose into an exfiltration instruction only inside the agent's working context. The evaluation point has to be the aggregate immediately before action (PG-2.11), not the arrivals.

**Letting content lose its origin on the way into the context.** Once a page, an email, and the user's own request are flattened into one context window, nothing distinguishes what the user asked for from what an attacker wrote. That is precisely how *PleaseFix* (August 2026) hijacked five agentic browsers, and it is why PG-2.5 provenance must tag origin, not just source type: an action has to be authorisable against the origin that requested it, and a cross-origin action inside one session needs explicit human confirmation.
