...
# Anticipated Changes to AI and Framework: MASO (Multi-Agent Security Operations) 2.0
...
ET-18, ET-19, and ET-20, together with vectors 1 (Judge Ceiling) and 4 (Multi-Agent Emergent Behaviours), point to the same architectural fix: stop routing every agent-to-agent hop through a single, central Model-as-Judge. [Distributed Security Architecture](distributed-architecture.md) describes the result: Layer 2 becomes a combination of reviewing controls, a per-agent security sidecar (deterministic scanners and purpose-built classifiers, <50ms, Model-as-Judge as the third control for escalations), a hardened inter-agent bus (schema enforcement and jailbreak-contagion detection), and Agent-to-Agent IAM with action gating (separation of duties between Read and Execute agents).
